...
def handle_dialog(res, req):
    ...
    if sessionStorage[user_id]['original_message'] is None:
        if req['request']['original_utterance'].lower() == 'выход':
            ...
            res['response']['text'] = 'Навык "Испорченный телефон", переводя ваше сообщение на разные языки, выдаёт интересный результат, поверьте, это весело! Введите любое предложение или слово, чтобы опробовать этот навык. Навык расчитан на то, что вы будете использовать кнопки с предлагаемыми ответами.'
            res['response']['buttons'] = [
                {
                    'title': 'Выход',
                    'hide': True
